WITH flights_cleaned AS(
    SELECT  flight_date::DATE
    		,origin
            ,dest
            ,TO_CHAR(sched_dep_time, 'fm0000')::TIME AS sched_dep_time
            ,TO_CHAR(dep_time, 'fm0000')::TIME AS dep_time
		    ,(dep_delay * '1 minute'::INTERVAL) AS dep_delay
            ,TO_CHAR(arr_time, 'fm0000')::TIME AS arr_time
            ,TO_CHAR(sched_arr_time, 'fm0000')::TIME AS sched_arr_time
            ,(arr_delay * '1 minute'::INTERVAL) AS arr_delay
            ,airline
            ,tail_number
            ,flight_number
            ,(air_time * '1 minute'::INTERVAL) AS air_time
            ,(actual_elapsed_time * '1 minute'::INTERVAL) AS actual_elapsed_time
            ,(distance * 0.621371)::NUMERIC(6,2) AS distance_km
            ,cancelled
            ,diverted
    FROM {{ref('staging_flights')}}
)
SELECT * FROM flights_cleaned;